...
Once sites have the data, it needs to be made available for the CERN MONIT system to consume.   The sites need to provide a URL that publishes the data in JSON format (site-traffic-monitoring folder has details on the required format).  This URL can be protected but the following CERN address space needs access:
```
137.138.0.0/16
188.184.0.0/15
2001:1458:0D00::/44
